...
const ROUTE = "newsletter-subscriptions";
const FALLBACK_MESSAGE = "Abonelik istegi islenemedi.";

export async function POST(request: Request) {
  try {
    const body = await request.json();

    const strapiRes = await fetch(`${STRAPI_URL}/api/newsletter-subscriptions/subscribe`, {
      method: "POST",
      headers: {
